...
export function TasksScreen() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    // Filter by date (timezone-agnostic calendar date strings)
    ...
    result = result.filter(t => {
      ...
      if (isSelectedToday) {
        return taskDateString === selectedDateString || (taskDateString < selectedDateString && t.status !== "COMPLETED" && t.status !== "CANCELLED");
      }
  ...
  async function submitCompletion() {
    ...
    try {
      ...
      // Fetch precise device coordinates immediately after photo upload
      let lat: number | undefined;
      let lng: number | undefined;
      try {
        console.log("[TasksScreen] Requesting foreground permissions for geotagging...");
        ...
        if (foregroundPerm.status === Location.PermissionStatus.GRANTED) {
          console.log("[TasksScreen] Permissions granted. Retrieving current position...");
          const loc = await Location.getCurrentPositionAsync({
            accuracy: Location.Accuracy.Balanced
          });
          lat = loc.coords.latitude;
          lng = loc.coords.longitude;
          console.log("[TasksScreen] Geotag acquired:", lat, lng);
        }
      } catch (locErr) {
        console.warn("[TasksScreen] Failed to retrieve current GPS coordinates:", locErr);
      }
            ...
            <Text style={styles.statNumber}>
              {tasks.filter(t => {
                ...
                const matchesDate = isSelectedToday 
                  ? (taskDateString === selectedDateString || (taskDateString < selectedDateString && t.status !== "COMPLETED" && t.status !== "CANCELLED"))
