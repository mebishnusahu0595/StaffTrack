...
import { Button, Card, Text, TouchableRipple, Icon, Dialog, Portal, IconButton, Modal, TextInput, Avatar, Badge } from "react-native-paper";
...
import { fetchDayEndReports, fetchExpenses, fetchTodayLocationLogs, uploadPhoto, fetchNotifications, markNotificationAsRead, fetchMonthlyPerformanceReport, type LocationPing, type PunchType } from "../api";
...
export function HomeScreen() {
  ...
  // Odometer reading prompt states
  ...
  async function handleCheckIn(type: PunchType) {
    ...
    try {
      ...
      let startOdometerPhotoUrl: string | undefined = undefined;
      let startOdometer: number | undefined = undefined;
      if (type === "FIELD") {
        const odoAsset = await pickOdometerImage("Start");
        if (!odoAsset) {
          console.log("[Punch] Odometer image picking cancelled. Cancelling check-in.");
          return;
        }

        const reading = await promptOdometerReading("Start");
        if (reading === null) {
          console.log("[Punch] Odometer reading input cancelled. Cancelling check-in.");
          return;
        }
  ...
  async function handleCheckOut() {
    ...
    try {
      ...
      let endOdometerPhotoUrl: string | undefined = undefined;
      let endOdometer: number | undefined = undefined;
      if (isField) {
        const odoAsset = await pickOdometerImage("End");
        if (!odoAsset) {
          console.log("[Punch] Odometer image picking cancelled. Cancelling check-out.");
          return;
        }

        const reading = await promptOdometerReading("End");
        if (reading === null) {
          console.log("[Punch] Odometer reading input cancelled. Cancelling check-out.");
          return;
        }
        endOdometer = reading;
    ...
    <ScrollView contentContainerStyle={styles.content} style={styles.screen}>
                ...
                <View style={{ marginTop: 20 }}>
                  ...
                  {isFieldPunch && activeAttendance?.startOdometerPhotoUrl ? (
                    <View style={styles.odometerSmallContainer}>
                      <HomeLoadingImage
                        resizeMode="cover"
                        source={{ uri: activeAttendance.startOdometerPhotoUrl.startsWith("http") ? activeAttendance.startOdometerPhotoUrl : `${API_ORIGIN_URL}${activeAttendance.startOdometerPhotoUrl}` }}
                        style={styles.odometerSmallPhoto}
                      />
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", backgroundColor: "#F8FAFC", borderRadius: 8, padding: 8, marginBottom: 12 }}>
              <View style={{ alignItems: "center", flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: "#0F172A", fontWeight: "800", marginTop: 2 }}>
                  {todayAttendance.startOdometer !== undefined && todayAttendance.startOdometer !== null ? `${todayAttendance.startOdometer} km` : "Not Recorded"}
                </Text>
              </View>
              ...
              <View style={{ alignItems: "center", flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: "#0F172A", fontWeight: "800", marginTop: 2 }}>
                  {todayAttendance.endOdometer !== undefined && todayAttendance.endOdometer !== null ? `${todayAttendance.endOdometer} km` : "Not Recorded"}
                </Text>
              </View>
              {todayAttendance.startOdometer !== undefined && todayAttendance.startOdometer !== null && todayAttendance.endOdometer !== undefined && todayAttendance.endOdometer !== null ? (
                <>
                  <View style={{ width: 1, backgroundColor: "#E2E8F0" }} />
                  <View style={{ alignItems: "center", flex: 1 }}>
                    <Text style={{ fontSize: 9, color: "#64748B", fontWeight: "700" }}>KM TRAVELLED</Text>
                    <Text style={{ fontSize: 12, color: "#0F172A", fontWeight: "800", marginTop: 2 }}>
                      {Math.max(0, todayAttendance.endOdometer - todayAttendance.startOdometer).toFixed(1)} km
                    </Text>
                  </View>
                </>
              ) : null}
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              {todayAttendance.startOdometerPhotoUrl ? (
                <View style={{ flex: 1, alignItems: "center" }}>
                  <HomeLoadingImage
                    resizeMode="cover"
                    source={{ uri: todayAttendance.startOdometerPhotoUrl.startsWith("http") ? todayAttendance.startOdometerPhotoUrl : `${API_ORIGIN_URL}${todayAttendance.startOdometerPhotoUrl}` }}
              ...
              {todayAttendance.endOdometerPhotoUrl ? (
                <View style={{ flex: 1, alignItems: "center" }}>
                  <HomeLoadingImage
                    resizeMode="cover"
                    source={{ uri: todayAttendance.endOdometerPhotoUrl.startsWith("http") ? todayAttendance.endOdometerPhotoUrl : `${API_ORIGIN_URL}${todayAttendance.endOdometerPhotoUrl}` }}
                    style={{ width: "100%", height: 110, borderRadius: 6, borderWidth: 1, borderColor: "#E2E8F0" }}
                  />
      ...
      <Portal>
        ...
        >
          ...
          <Dialog.Content>
            <Text style={{ fontSize: 13, color: '#66736F', marginBottom: 12 }}>
          ...
          <Dialog.Actions>
            ...
            <Button 
              mode="contained"
              disabled={!odoValue.trim() || isNaN(Number(odoValue))}
              onPress={() => {
                const val = Number(odoValue.trim());
                setOdoModalVisible(false);
                if (odoResolve) {
                  odoResolve.resolve(val);
                }
              }}
            >
              Submit
...
async function pickVerificationImage(options?: {
  quality?: number;
...
function HomeLoadingImage({ source, style, resizeMode }: { source: any; style: any; resizeMode?: "cover" | "contain" | "stretch" | "center" }) {
    ...
    <View style={[{ position: "relative", overflow: "hidden", backgroundColor: "#F1F5F9" }, style]}>
      ...
      {loading && (
        <View style={[StyleSheet.absoluteFill, { justifyContent: "center", alignItems: "center", backgroundColor: "#F1F5F9" }]}>
          ...
          <Text style={{ fontSize: 10, color: "#66736F", marginTop: 6 }}>Loading...</Text>
        </View>
      )}
      {error && (
