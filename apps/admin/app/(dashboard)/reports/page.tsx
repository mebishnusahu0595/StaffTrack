...
export default function ReportsPage() {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      headers.join(","),
      ...rowsData.map(row => row.map(val => `"${(val ?? "").toString().replace(/"/g, '""')}"`).join(","))
    ].join("\n");
                          ...
                          <div className="px-24 py-12 grid gap-12 md:grid-cols-2 animate-in fade-in zoom-in-95 duration-500">
                            <div className="relative">
                               ...
                               <div className="space-y-4">
                                  ...
                                  {(row.startOdometerPhotoUrl || row.kmPhotoUrl) && (
                                    <div className="mt-6 space-y-4">
                                      <div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 flex items-center gap-3">
                                         Odometer Audit Verification
                                         <div className="h-px flex-1 bg-emerald-100" />
                                      </div>
                                      <div className="flex flex-row justify-center items-center gap-4">
                                        {row.startOdometerPhotoUrl && (
                                          <div className="flex flex-col items-center gap-2">
                                            <PhotoViewer 
                                              url={row.startOdometerPhotoUrl} 
                                              label={`Start Day Odometer ${row.startOdometer ? `(${row.startOdometer} KM)` : ""}`} 
                                              trigger={
                                                <div className="relative group w-48 h-32 rounded-2xl overflow-hidden border-2 border-amber-100 cursor-pointer shadow-lg hover:ring-4 hover:ring-amber-500/20 transition-all">
                                                  <img src={row.startOdometerPhotoUrl} className="w-full h-full object-cover" alt="Start Odometer" />
                                            ...
                                            />
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Start Odometer{row.startOdometer ? `: ${row.startOdometer} KM` : ""}</span>
                                          </div>
                                        )}
                                        {row.kmPhotoUrl && (
                                          <div className="flex flex-col items-center gap-2">
                                            <PhotoViewer 
                                              url={row.kmPhotoUrl} 
                                              label={`End Day Odometer ${row.endOdometer ? `(${row.endOdometer} KM)` : ""}`} 
                                              trigger={
                                                <div className="relative group w-48 h-32 rounded-2xl overflow-hidden border-2 border-emerald-100 cursor-pointer shadow-lg hover:ring-4 hover:ring-emerald-500/20 transition-all">
                                                  <img src={row.kmPhotoUrl} className="w-full h-full object-cover" alt="End Odometer" />
                                                  <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                    <Camera className="h-6 w-6 text-white" />
                                                  </div>
                                                </div>
                                            ...
                                            />
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">End Odometer{row.endOdometer ? `: ${row.endOdometer} KM` : ""}</span>
                                          </div>
                                        )}
                                      </div>
                                    </div>
