...
import { createUser, fetchAttendance, fetchTodayLocation, fetchUsers, updateUser, createTask, deleteUser, fetchTasks, fetchDerHistory, fetchGroups } from "@/lib/api";
import { formatTime } from "@/lib/format";
import { cn } from "@/lib/utils";
import type { WorkMode, User } from "@/lib/types";
...
export default function EmployeesPage() {
  ...
  const usersQuery = useQuery({ 
    queryKey: ["users", search, workModeFilter], 
    queryFn: () => fetchUsers({ page: 1, pageSize: 100, search, workMode: workModeFilter === "ALL" ? undefined : workModeFilter }) 
  });
  ...
  const isFieldEmployee = selectedEmployee?.workMode === "FIELD";
 
  // Fetch details for the expanded employee
  const locationQuery = useQuery({
    queryKey: ["location", expandedId, "today"],
    queryFn: () => fetchTodayLocation(expandedId!),
    enabled: Boolean(expandedId && isFieldEmployee),
    refetchInterval: expandedId && isFieldEmployee ? 20_000 : false,
  });
 
  const attendanceQuery = useQuery({
    queryKey: ["attendance", expandedId],
    queryFn: () => fetchAttendance(expandedId!),
    enabled: !!expandedId,
    refetchInterval: expandedId ? 20_000 : false,
  });
  ...
  const latestAttendance = useMemo(() => {
    ...
    return (attendanceQuery.data ?? []).find((record) => new Date(record.date).toDateString() === today);
                  ...
                  <Fragment key={user.id}>
                    <tr className={cn(
                      ...
                      <td className="py-5 px-6 text-xs font-semibold text-slate-700">
                        {user.managerId ? managersById.get(user.managerId)?.name ?? "Unknown manager" : "Unassigned"}
                      </td>
                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={6} className="p-0 border-none bg-slate-50/80">
                                       ...
                                       <CardContent className="p-6">
                                          ...
                                          <div className="mt-8 grid grid-cols-2 gap-4">
                                             <div className="p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Today Attendance</p>
                                                <div className="flex items-end gap-1 mb-1">
                                                   <span className="text-lg font-black text-slate-900 leading-none">{latestAttendance?.checkInTime ? "In" : "--"}</span>
                                                   <span className="text-[10px] font-bold text-slate-400 uppercase leading-none">{latestAttendance?.checkOutTime ? "/ Out" : ""}</span>
                                                </div>
                                                <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                                   <div className={cn("h-full rounded-full", latestAttendance?.checkInTime ? "w-full bg-blue-600" : "w-0 bg-blue-600")} />
                                                </div>
                                             </div>
                                             <div className="p-3 rounded-xl bg-slate-50/50 border border-slate-100">
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">{isFieldEmployee ? "Distance" : "Work Mode"}</p>
                                                <div className="flex items-end gap-1 mb-1">
                                                   <span className="text-lg font-black text-slate-900 leading-none">{isFieldEmployee ? distanceKm.toFixed(1) : "Office"}</span>
                                 ...
                                 <div className="col-span-6">
                                    <Card className="border-none shadow-sm ring-1 ring-slate-200/50 h-full relative overflow-hidden bg-slate-200">
                                       <div className="absolute top-4 left-4 z-10">
                                          <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white shadow-sm ring-1 ring-slate-200/20">
                                             <div className={cn("h-2 w-2 rounded-full", isFieldEmployee ? "bg-emerald-500 animate-pulse" : "bg-indigo-500")} />
                                       ...
                                       <div className="w-full h-full bg-[#E5E7EB] flex items-center justify-center p-10">
                                          <div className="w-full max-w-md rounded-2xl bg-white/90 p-6 text-center shadow-sm ring-1 ring-slate-200">
                                             ...
                                             <p className="mt-1 text-xs font-bold text-slate-500">
                                               {isFieldEmployee
                                                 ? (latestLocation ? `${latestLocation.lat.toFixed(5)}, ${latestLocation.lng.toFixed(5)}` : "No location logs received today.")
                                                 : (latestAttendance?.checkInTime ? `Checked in ${formatTime(latestAttendance.checkInTime)}` : "No office check-in today.")}
                                             </p>
                                                ...
                                                <TabsContent value="attendance" className="m-0 space-y-6">
                                                   <div className="flex items-start gap-4">
                                                      ...
                                                      <div className="flex-1">
                                                         ...
                                                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mt-1">{latestAttendance?.checkInTime ? formatTime(latestAttendance.checkInTime) : "No check-in"}</p>
                                                      </div>
                                                   </div>
                                                   <div className="flex items-start gap-4">
                                                      ...
                                                      <div className="flex-1">
                                                         ...
                                                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mt-1">{latestAttendance?.checkOutTime ? formatTime(latestAttendance.checkOutTime) : "Not checked out"}</p>
                                                      </div>
                                                   </div>
                                                   <div className="flex items-center justify-between rounded-xl bg-slate-50 p-3">
                                                      ...
                                                      <AttendanceStatusBadge
                                                        ...
                                                        hasCheckOut={Boolean(latestAttendance?.checkOutTime)}
                                                        checkInTime={latestAttendance?.checkInTime ?? undefined}
    ...
    try {
      if (mode === "CREATE") {
        await createUser({ 
          ...
          designation, joiningDate: new Date(joiningDate),
        ...
        await updateUser(user.id, {
          name, email, phone, workMode, designation, joiningDate: new Date(joiningDate),
        ...
        <div className="flex-1 p-10 max-h-[85vh] overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              {mode === "CREATE" && <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Password *</Label><Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="h-11 rounded-xl" /></div>}
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Assign Group</Label>
                <Select value={groupId} onValueChange={setGroupId}>
                   <SelectTrigger className="h-11 rounded-xl"><SelectValue placeholder="No Group" /></SelectTrigger>
                   <SelectContent>
                      <SelectItem value="none">No Group (Independent)</SelectItem>
