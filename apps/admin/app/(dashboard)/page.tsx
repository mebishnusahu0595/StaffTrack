...
export default function OverviewPage() {
  ...
  // ─── Reusable Components ──────────────────────────────────────────────────
  function StatCard({ icon: Icon, title, color, details }: { icon: any; title: string; color: string; details: { label: string; value: number }[] }) {
    return (
      <Card className="border-none shadow-sm ring-1 ring-slate-200/60 p-5 rounded-3xl">
        <div className="flex items-center gap-3 mb-4">
          <div className={cn("p-2 rounded-xl", color === "blue" ? "bg-blue-50 text-blue-600" : color === "amber" ? "bg-amber-50 text-amber-600" : color === "rose" ? "bg-rose-50 text-rose-600" : "bg-indigo-50 text-indigo-600")}>
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4">
          {details.map(d => (
            <div key={d.label}>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{d.label}</div>
  ...
  function TabItem({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) {
    return (
      <button onClick={onClick} className={cn("flex items-center gap-2 pb-3 border-b-2 transition-colors", active ? "border-blue-600" : "border-transparent")}>
        <span className={cn("text-xs font-black uppercase tracking-wider", active ? "text-blue-600" : "text-slate-400")}>{label}</span>
        <span className={cn("px-1.5 py-0.5 rounded text-[9px] font-bold", active ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-500")}>{count}</span>
      </button>
    );
  }
