...
export function middleware(request: NextRequest) {
  ...
  const isAuthPage = pathname === "/login" || pathname === "/superadmin-login";
  ...
  if (!canAccess && !isAuthPage && !isPublicForm) {
    const redirectPath = pathname.includes("super") ? "/superadmin-login" : "/login";
    return NextResponse.redirect(new URL(redirectPath, request.url));
  }

  if (canAccess && pathname === "/login") {
    const dest = user?.role === "SUPERADMIN" ? "/super-dashboard" : "/";
