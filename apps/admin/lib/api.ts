...
export async function createProject(data: { name: string; description?: string; status?: string }) {
...
export async function fetchTodayLocation(userId: string) {
  const response = await api.get<{ data: LocationLog[] }>(`/location/${userId}/today`);
  return response.data.data;
}
